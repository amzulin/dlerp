...
namespace Enterprise.Invoicing.Common
{
    ...
    public class Utils
    {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public static bool DayMonthLastDay(DateTime dt)
        {
            int month1 = dt.Month;
            if (dt.AddDays(-1).Month != month1)
            {
                return true;
            }
            if (dt.AddDays(-2).Month != month1)
            {
                return true;
            } if (dt.AddDays(-3).Month != month1)
            {
                return true;
            }
            if (dt.AddDays(-4).Month != month1)
            {
                return true;
            } if (dt.AddDays(-5).Month != month1)
            {
                return true;
            }
            return false;
            //int month2 = dt.AddDays(1).Month;
            //return month1 != month2;
        }
        public static bool DayMonthShow(DateTime dt, int lastday)
        {
            if (dt.Day > lastday)
            {
                return true;
        ...
        /// <returns></returns>
        public static bool DayIsWeekDay(DateTime dt, string weekday)
        {
            return true;
            //string week1 = Convert.ToString(dt.DayOfWeek);//星期
            //return week1.ToLower() == weekday.ToLower();
        }

        /// <summary>
        /// 根据ip返回flex地址
        /// http://localhost:13773/WebServices/FlexServices.svc?wsdl
        /// http://192.168.1.200/WebServices/FlexServices.svc?wsdl
        /// http://ecobio-wms.vicp.cc/WebServices/FlexServices.svc?wsdl
        /// </summary>
        /// <returns></returns>
        public static string GetFlexAddress()
        {
            string ip = "";
            ...
            if (ip == "::1")
            {
                return "http://localhost:13773/WebServices/FlexServices.svc?wsdl";
            }
            else if (ip.StartsWith("192.168.1.200"))
            {
                return "http://192.168.1.200/WebServices/FlexServices.svc?wsdl";
            }
            else if (ip.StartsWith("192.168.1.104"))
            {
                return "http://192.168.1.104/WebServices/FlexServices.svc?wsdl";
            }
            else
            {
                //return "http://ecobio-wms.vicp.cc/WebServices/FlexServices.svc?wsdl";
                return "http://" + host + "/WebServices/FlexServices.svc?wsdl";
            }
        }
        /// <summary>
        /// 获得请求主机的地址http:// + host + /
        ...
        /// </summary>
        /// <returns></returns>
        public static int GetWeekOfYear(DateTime now)
        {
            //一.找到第一周的最后一天（先获取1月1日是星期几，从而得知第一周周末是几）
            int firstWeekend = 7 - Convert.ToInt32(DateTime.Parse(now.Year + "-1-1").DayOfWeek);
        ...
        /// <returns></returns>
        public static string GetStatus(int key, int status)
        {
            if (key == 1)
            {
                #region 采购申请
        ...
        /// <param name="status"></param>
        /// <returns></returns>
        public static string GetBtnStr(int status)
        {
            #region 采购申请
            switch (status)
            {
                case 0: return "<span title='审核'>审核</span>";
                case 1: return "<span title='返审' >返审</span>";
                case 4: return "<span title='撤消作废'>撤消</span>";
                default: return "<span title='操作未知'>未知</span>";
            }
            #endregion
        }  
        public static string GetbillBtnStr(int status)
        {
            #region 采购申请
            switch (status)
            {
                case 0: return "<span title='审核'>审核</span>";
                case 1: return "<span title='审核' >审核</span>";
                case 8: return "<span title='审批'>审批</span>";
                case 9: return "<span title='完结'>完结</span>";
                default: return "<span title='操作未知'>未知</span>";
            }
            #endregion
        }
        ...
        /// <returns></returns>
        public static string GetInName(int intype)
        {
            switch (intype)
            {
                case 1: return "采购入库";
        ...
        /// <param name="intype"></param>
        /// <returns></returns>
        public static string GetOutName(int outtype)
        {
            switch (outtype)
            {
                case 1: return "销售出库";
        ...
        public static string GetStockName(string key, int type)
        {
        ...
        public static String NumGetStr(Double Num)
        {
            ...
            if (iZhSh_bool)
            {//整数部分处理
                NumStr_Zh = Reverse(NumStr_Zh);//反转字符串
